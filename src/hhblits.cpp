...
void HHblits::getTemplateA3M(HHblitsDatabase* db, char* entry_name,
	...
	if (db->use_compressed) {
		...
		char* data = ffindex_get_data_by_entry(db->ca3m_database->db_data,
				entry);
...
void HHblits::perform_realign(std::vector<HHDatabaseEntry*>& hits_to_realign,
	...
	while (!hitlist.End()) {
		...
		if (nhits >= premerge) {
			if (hit_cur.irep == 1) {
				...
				if (!phash_plist_realignhitpos->Contains(
						hit_cur.entry->entry->name)) {
					List<Realign_hitpos>* newlist = new List<Realign_hitpos>;
					phash_plist_realignhitpos->Add(hit_cur.entry->entry->name,
							newlist);
				}
	...
	if (premerge > 0) {
		...
		while (!hitlist.End() && nhits < premerge) {
			...
			HHblitsDatabase* db = getHHblitsDatabase(*hit_cur.entry, dbs);
			if (db == NULL) {
				std::cerr << "Could not find database for premerge!"
						<< std::endl;
				continue;
			}
...
#pragma omp parallel for schedule(dynamic, 1)
	for (size_t idb = 0; idb < hits_to_realign.size(); idb++) {
		...
		if (!phash_plist_realignhitpos->Contains(
				hits_to_realign[idb]->entry->name))
			continue;
...
void HHblits::writeReducedHHRFile(HHblits& hhblits, std::stringstream& out) {
	hhblits.reducedHitlist.PrintHHR(hhblits.q_tmp, out, hhblits.par.maxdbstrlen,
			hhblits.par.showconf, hhblits.par.showcons, hhblits.par.showdssp,
			hhblits.par.showpred, hhblits.par.b, hhblits.par.B, hhblits.par.z,
			hhblits.par.Z, hhblits.par.aliwidth, hhblits.par.nseqdis,
			hhblits.par.p, hhblits.par.E, hhblits.par.argc, hhblits.par.argv,
