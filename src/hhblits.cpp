...
HHblits::HHblits(Parameters& parameters,
	...
	if (!par.nocontxt && *par.clusterfile) {
		...
		fclose(fin);
		pc_hhm_context_engine->SetTargetNeff(
				par.pc_hhm_context_engine.target_neff);
		...
		pc_prefilter_context_mode =
				par.pc_prefilter_context_engine.CreateAdmix();
	}
...
void HHblits::mergeHitsToQuery(Hash<Hit>* previous_hits,
		Hash<char>* premerged_hits, int& seqs_found, int& cluster_found) {
...
//  q->Log2LinTransitionProbs(1.0); // transform transition freqs to lin space if not already done
	...
	HH_LOG(LogLevel::INFO) << "Realigning " << nhits
			<< " HMM-HMM alignments using Maximum Accuracy algorithm"
			<< std::endl;
...
void HHblits::optimizeQSC(std::vector<HHEntry*>& selected_entries,
	...
	const int cov_tot = std::max(
			std::min((int) (COV_ABS / q->L * 100 + 0.5), 70), par.coverage);
	...
	for (int i = 0; i < nqsc; i++) {
		...
		ViterbiRunner viterbirunner(viterbiMatrices, dbs, par.threads);
		std::vector<Hit> hits_to_add = viterbirunner.alignment(par, &q_vec,
				selected_entries, actual_qsc, pb, S, Sim, R);
	  ...
	  for (int i = 0; i < par.threads; i++) {
		...
		std::map<short int, std::vector<Hit *> > alignments;
		for (int elem = 0; elem < (int) hit_vector.size(); elem++) {
			alignments[hit_vector.at(elem)->irep].push_back(
					hit_vector.at(elem));
		}

		PosteriorDecoderRunnerInputData input_data(dbs, hits_to_realign,
				alignments, n_realignments, t_maxres);
...
void HHblits::get_entries_of_selected_hits(HitList& input, std::vector<HHEntry*>& selected_entries) {
	std::set<std::string> output_set;
	...
	while (!input.End()) {
		...
		if (n_realignments >= par.realign_max && n_realignments >= imax(par.B, par.Z))
			break;
...
void HHblits::get_entries_of_all_hits(HitList& input, std::vector<HHEntry*>& selected_entries) {
	std::set<std::string> output_set;
...
void HHblits::run(FILE* query_fh, char* query_path) {
	...
	for (int round = 1; round <= par.num_rounds; round++) {
		...
		if (premerge > 0 && round > 1 && previous_hits->Size() >= premerge) {
			HH_LOG(LogLevel::DEBUG1) << "Set premerge to 0! (premerge: "
					<< premerge << "   iteration: " << round << "   hits.Size: "
...
//    }
		...
		if (par.prefilter) {
			...
			for (size_t i = 0; i < dbs.size(); i++) {
				dbs[i]->prefilter_db(q_tmp, previous_hits, par.threads,
						...
						par.preprefilter_smax_thresh, par.min_prefilter_hits,
						par.maxnumdb, R, new_entries, old_entries);
			}
		...
		if (max_template_length > par.maxres) {
			HH_LOG(LogLevel::WARNING)
					...
					<< par.maxres
					<< "). Maxres can be increased with parameter -maxres."
					<< std::endl;
		}
		max_template_length = std::min(max_template_length, par.maxres);
		for (int i = 0; i < par.threads; i++) {
			viterbiMatrices[i]->AllocateBacktraceMatrix(q->L,
					max_template_length);
		}
		...
		if (new_entries.size() == 0) {
			HH_LOG(LogLevel::INFO)
					<< "No HMMs pass prefilter => Stop searching!" << std::endl;
			break;
		}
		...
		ViterbiRunner viterbirunner(viterbiMatrices, dbs, par.threads);
		std::vector<Hit> hits_to_add = viterbirunner.alignment(par, &q_vec,
				new_entries, par.qsc_db, pb, S, Sim, R);
		...
		if (new_hits == 0 || round == par.num_rounds) {
			...
			if (old_entries.size() > 0 && par.realign_old_hits) {
				...
				ViterbiRunner viterbirunner(viterbiMatrices, dbs, par.threads);
				std::vector<Hit> hits_to_add = viterbirunner.alignment(par,
						&q_vec, old_entries, par.qsc_db, pb, S, Sim, R);
...
//				// Add dbfiles_old to dbfiles_new for realign
		...
		if (round < par.num_rounds || *par.alnfile || *par.psifile
			...
			if (par.addss
					&& (*par.alisbasename || round == par.num_rounds
							|| new_hits == 0)) {
		...
		HH_LOG(LogLevel::INFO) << seqs_found << " sequences belonging to "
				<< cluster_found << " database HMMs found with an E-value < "
				<< par.e << std::endl;
		...
		if (q->Neff_HMM > par.neffmax && round < par.num_rounds) {
			HH_LOG(LogLevel::INFO) << "Diversity is above threshold ("
					<< par.neffmax
...
void HHblits::writeHHRFile(char* hhrFile) {
	if (*hhrFile) {
		hitlist.PrintHHR(q_tmp, hhrFile, par.maxdbstrlen, par.showconf,
				par.showcons, par.showdssp, par.showpred, par.b, par.B, par.z,
				par.Z, par.aliwidth, par.nseqdis, par.p, par.E, par.argc,
				par.argv, S);
	}
}
...
void HHblits::writePairwiseAlisFile(char* pairwiseAlisFile, char outformat) {
	if (*pairwiseAlisFile) {
		hitlist.PrintAlignments(q, pairwiseAlisFile, par.showconf, par.showcons,
				par.showdssp, par.showpred, par.p, par.aliwidth, par.nseqdis,
				par.b, par.B, par.E, S, outformat);
	}
}
...
void HHblits::writeOptimizedHHRFile(char* reducedHHRFile) {
	if (*reducedHHRFile) {
		optimized_hitlist.PrintHHR(q_tmp, reducedHHRFile, par.maxdbstrlen,
				par.showconf, par.showcons, par.showdssp, par.showpred, par.b,
				par.B, par.z, par.Z, par.aliwidth, par.nseqdis, par.p, par.E,
				par.argc, par.argv, S);
	}
}
...
void HHblits::writeHMMFile(char* HMMFile) {
	...
	if (*HMMFile) {
		...
		q->WriteToFile(HMMFile, par.append, par.max_seqid, par.coverage,
...
void HHblits::writeHHRFile(HHblits& hhblits, std::stringstream& out) {
	hhblits.hitlist.PrintHHR(hhblits.q_tmp, out, hhblits.par.maxdbstrlen,
			hhblits.par.showconf, hhblits.par.showcons, hhblits.par.showdssp,
			hhblits.par.showpred, hhblits.par.b, hhblits.par.B, hhblits.par.z,
			hhblits.par.Z, hhblits.par.aliwidth, hhblits.par.nseqdis,
			hhblits.par.p, hhblits.par.E, hhblits.par.argc, hhblits.par.argv,
			hhblits.S);
}
...
void HHblits::writePairwiseAlisFile(HHblits& hhblits, std::stringstream& out) {
	hhblits.hitlist.PrintAlignments(hhblits.q, out, hhblits.par.showconf,
			hhblits.par.showcons, hhblits.par.showdssp, hhblits.par.showpred,
			hhblits.par.p, hhblits.par.aliwidth, hhblits.par.nseqdis,
			hhblits.par.b, hhblits.par.B, hhblits.par.E, hhblits.S,
...
void HHblits::writeAlitabFile(HHblits& hhblits, std::stringstream& out) {
	hhblits.hitlist.WriteToAlifile(hhblits.q, out, hhblits.par.b, hhblits.par.B,
			hhblits.par.z, hhblits.par.Z, hhblits.par.p, hhblits.par.E);
}
...
void HHblits::writeOptimizedHHRFile(HHblits& hhblits, std::stringstream& out) {
	hhblits.optimized_hitlist.PrintHHR(hhblits.q_tmp, out,
			hhblits.par.maxdbstrlen, hhblits.par.showconf, hhblits.par.showcons,
			hhblits.par.showdssp, hhblits.par.showpred, hhblits.par.b,
			hhblits.par.B, hhblits.par.z, hhblits.par.Z, hhblits.par.aliwidth,
			hhblits.par.nseqdis, hhblits.par.p, hhblits.par.E, hhblits.par.argc,
...
void HHblits::writeMatricesFile(char* matricesOutputFileName) {
	if (*matricesOutputFileName) {
		hitlist.PrintMatrices(q, matricesOutputFileName,
				par.max_number_matrices, S);
	}
}

void HHblits::writeMatricesFile(HHblits& hhblits, stringstream& out) {
	hhblits.hitlist.PrintMatrices(hhblits.q, out,
			hhblits.par.max_number_matrices, hhblits.S);
}
