...
void HHblits::getTemplateA3M(HHblitsDatabase* db, char* entry_name, long& ftellpos, Alignment& tali) {
	...
	tali.Compress(entry_name, par.cons, par.maxres, par.maxcol, par.M,
			par.Mgaps);
}

HHblitsDatabase* HHblits::getHHblitsDatabase(HHDatabaseEntry& entry, std::vector<HHblitsDatabase*>& dbs) {
	int superID = entry.ffdatabase->superId;
	...
	if (entry.ffdatabase->isCompressed) {
		...
		if (dbf != NULL) {
			...
			// read a3m alignment
				...
				Alignment tali;
				tali.Read(dbf, entry.entry->name, par.mark, par.maxcol, par.nseqdis, line);
				tali.Compress(entry.entry->name, par.cons, par.maxres, par.maxcol, par.M, par.Mgaps);
...
void HHblits::DoViterbiSearch(std::vector<HHDatabaseEntry*>& prefiltered_hits,
	...
	for (int idb = 0; idb < prefiltered_hits.size(); idb++) {
		...
		if (!(par.early_stopping_filter && par.filter_sum < filter_cutoff)) {
			...
			hit[bin]->dbfile = new char[strlen(prefiltered_hits[idb]->entry->name) + 1];
...
void HHblits::perform_realign(std::vector<HHDatabaseEntry*>& hits_to_realign,
	...
	if (par.premerge > 0) {
		...
		while (!hitlist.End() && nhits < par.premerge) {
			...
			hit[bin]->Forward(q, t[bin], par.ssm, par.min_overlap, par.loc,
					par.shift, par.ssw, par.exclstr, S73, S33);
			hit[bin]->Backward(q, t[bin], par.loc, par.shift, par.ssw, S73,
					S33);
			...
			Qali.N_filtered = Qali.Filter(par.max_seqid, S, par.coverage,
	...
	for (int idb = 0; idb < hits_to_realign.size(); idb++) {
		...
		while (!phash_plist_realignhitpos->Show(hits_to_realign[idb]->entry->name)->End()) {
			...
			while (!phash_plist_realignhitpos->Show(
					hits_to_realign[idb]->entry->name)->End()) {
				// Allocate free bin
				int bin = omp_get_thread_num();
				...
				hit[bin]->dbfile = new char[strlen(hits_to_realign[idb]->entry->name) + 1];
				...
				RealignByWorker(par, hit[bin], q, t[bin], format[bin], pb, R,
						S73, S33);
...
void HHblits::recalculateAlignmentsForDifferentQSC(HitList& hitlist,
	...
	for (size_t qsc_index = 0; qsc_index < nqsc; qsc_index++) {
		...
		while (!hitlist.End()) {
			...
			for (int irep = 1; irep <= par.altali; irep++) {
				...
				hit.Viterbi(q, t, par.loc, par.ssm, par.maxres, par.min_overlap,
						par.shift, par.egt, par.egq, par.ssw, par.exclstr, S73,
						S33);
		...
		realigned_viterbi_hitlist.CalculatePvalues(q, par.loc, par.ssm,
				par.ssw);
		realigned_viterbi_hitlist.CalculateHHblitsEvalues(q, par.dbsize,
				par.alphaa, par.alphab, par.alphac,
		...
		while (!realigned_viterbi_hitlist.End()) {
			...
			// Align q to template in *hit[bin]
			hit.Forward(q, t, par.ssm, par.min_overlap, par.loc, par.shift,
...
//    }
		...
		if (round < par.num_rounds || *par.alnfile || *par.psifile
			...
			if (new_hits != 0) {
				...
				// Convert ASCII to int (0-20),throw out all insert states, record their number in I[k][i]
				Qali.Compress("merged A3M file", par.cons, par.maxres,
						par.maxcol, par.M, par.Mgaps);

				// Sort out the nseqdis most dissimilacd r sequences for display in the result alignments
				Qali.FilterForDisplay(par.max_seqid, par.mark, S, par.coverage,
						par.qid, par.qsc, par.nseqdis);
				...
				Qali.N_filtered = Qali.Filter(par.max_seqid, S, cov_tot,
						par.qid, par.qsc, par.Ndiff);
			}
...
void HHblits::writeHHRFile(std::stringstream& out) {
	hitlist.PrintHHR(q_tmp, out, par.maxdbstrlen, par.showconf, par.showcons,
			par.showdssp, par.showpred, par.b, par.B, par.z, par.Z,
...
void HHblits::writePairwiseAlisFile(char outformat, std::stringstream& out) {
	hitlist.PrintAlignments(q, out, par.showconf, par.showcons, par.showdssp,
			par.showpred, par.p, par.aliwidth, par.nseqdis, par.b, par.B, par.E,
			S, outformat);
}
...
void HHblits::writeReducedHHRFile(std::stringstream& out) {
	reducedHitlist.PrintHHR(q_tmp, out, par.maxdbstrlen, par.showconf,
			par.showcons, par.showdssp, par.showpred, par.b, par.B, par.z,
			par.Z, par.aliwidth, par.nseqdis, par.p, par.E, par.argc, par.argv,
			S);
}
...
void HHblits::writeHMMFile(std::stringstream& out) {
	...
	q->WriteToFile(out, par.max_seqid, par.coverage, par.qid, par.Ndiff,
			par.qsc, par.argc, par.argv, pb);
}
