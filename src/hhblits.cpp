...
void HHblits::optimizeQSC(HitList& input_list, HMMSimd& q_vec,
  ...
  while (!input_list.End()) {
    ...
    for (int i = 0; i < nqsc; i++) {
      ...
      while (!tmp_list.End()) {
        ...
        std::cout << hit_cur.name << "_" << hit_cur.irep << "\t" << actual_qsc
            << "\t" << cur_ali_quality << std::endl;
  ...
  output_list.N_searched = input_list.N_searched;
}
...
//				// Add dbfiles_old to dbfiles_new for realign
    ...
    HH_LOG(LogLevel::INFO) << seqs_found << " sequences belonging to "
        << cluster_found << " database HMMs found with an E-value < " << par.e
        << std::endl;
    ...
    if (q->Neff_HMM > par.neffmax && round < par.num_rounds) {
      HH_LOG(LogLevel::INFO) << "Diversity is above threshold (" << par.neffmax
...
void HHblits::writeHHRFile(HHblits& hhblits, std::stringstream& out) {
  hhblits.hitlist.PrintHHR(hhblits.q_tmp, out, hhblits.par.maxdbstrlen,
      ...
      hhblits.par.Z, hhblits.par.aliwidth, hhblits.par.nseqdis, hhblits.par.p,
      hhblits.par.E, hhblits.par.argc, hhblits.par.argv, hhblits.S);
}
...
void HHblits::writePairwiseAlisFile(HHblits& hhblits, std::stringstream& out) {
  hhblits.hitlist.PrintAlignments(hhblits.q, out, hhblits.par.showconf,
      ...
      hhblits.par.p, hhblits.par.aliwidth, hhblits.par.nseqdis, hhblits.par.b,
      hhblits.par.B, hhblits.par.E, hhblits.S, hhblits.par.outformat);
}
...
void HHblits::writeOptimizedHHRFile(HHblits& hhblits, std::stringstream& out) {
  hhblits.optimized_hitlist.PrintHHR(hhblits.q_tmp, out, hhblits.par.maxdbstrlen,
      hhblits.par.showconf, hhblits.par.showcons, hhblits.par.showdssp,
      hhblits.par.showpred, hhblits.par.b, hhblits.par.B, hhblits.par.z,
      hhblits.par.Z, hhblits.par.aliwidth, hhblits.par.nseqdis, hhblits.par.p,
      hhblits.par.E, hhblits.par.argc, hhblits.par.argv, hhblits.S);
...
void HHblits::writeHMMFile(HHblits& hhblits, std::stringstream& out) {
  ...
  hhblits.q->WriteToFile(out, hhblits.par.max_seqid, hhblits.par.coverage,
      hhblits.par.qid, hhblits.par.Ndiff, hhblits.par.qsc, hhblits.par.argc,
      hhblits.par.argv, hhblits.pb);
}
