...
void init_no_prefiltering()
{
  ...
  if (db_index_file == NULL) OpenFileError(dbcs_index_filename);
...
void init_prefilter()
{
  ...
  if (db_data_file == NULL) OpenFileError(dbcs_data_filename);
  size_t db_data_size;
...
void stripe_query_profile()
{
  ...
  if (par.nocontxt) {
    ...
    q_tmp->AddAminoAcidPseudocounts(par.pc_prefilter_nocontext_mode, par.pc_prefilter_nocontext_a,
        par.pc_prefilter_nocontext_b, par.pc_prefilter_nocontext_c);
  } else {
