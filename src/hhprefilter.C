...
int swStripedByte(unsigned char *querySeq, int queryLength,
...
int swStripedWord_backtrace(int queryLength, unsigned char *dbSeq, int dbLength,
    unsigned short gapOpen, unsigned short gapExtend, __m128i *pvQueryProf,
    __m128i *pvHLoad, __m128i *pvHStore, __m128i *pvE, ali_pos *res) {
...
///////////////////////////////////////////////////////////////////////////////////
  ...
  __m128i vIter = _mm_setr_epi16(0, iter, 2 * iter, 3 * iter, 4 * iter,
      5 * iter, 6 * iter, 7 * iter);
  
  int num = 0;
  int crossout_thresh = (int) (par.block_shading_space / 1.3);
  double factor = (double) par.dbsize * (double) queryLength
      * (double) dbLength;
  ...
  while (fgetline(line, LINELEN, dbf)) // read HMM files in pal file
  {
    if (line[0] == '>') {
      ...
      if (ptr1) // found '|' in sequence id? => extract string up to '|'
      {
        ...
        if (ptr2)
          strmcpy(db_name, ptr1, ptr2 - ptr1);
        else
          strcpy(db_name, ptr1);
      }
      else
...
void init_prefilter() {
  ...
  if (v >= 2)
    cout << "Reading in " << par.dbsize
        << " column state sequences with a total of " << LDB << " residues"
        << endl;

  X = (unsigned char*) memalign(16, LDB * sizeof(unsigned char)); // database string (concatenate all DB-seqs)
  first = (unsigned char**) memalign(16,
      (par.dbsize + 2) * sizeof(unsigned char*)); // first characters of db sequences. Was (par.dbsize*2). Why??
...
void stripe_query_profile() {
  ...
  qc = (unsigned char*) memalign(16,
      (NUMCOLSTATES + 1) * (LQ + 15) * sizeof(unsigned char)); // query profile (states + 1 because of ANY char)
  ...
  qw = (unsigned short*) memalign(16,
      (NUMCOLSTATES + 1) * (LQ + 7) * sizeof(unsigned short)); // query profile (states + 1 because of ANY char)
  Ww = (LQ + 7) / 8;
...
#pragma omp parallel for schedule(static) private(evalue, score, thread_id)
  for (int n = 0; n < count_dbs; n++)     // Loop over all database sequences
    ...
    // Perform search step
    score = swStripedByte(qc, LQ, first[prefiltered_hits[n]],
        length[prefiltered_hits[n]], gap_init, gap_extend, workspace[thread_id],
        workspace[thread_id] + W, workspace[thread_id] + 2 * W,
...
#pragma omp parallel for schedule(static) private(block, block_count, thread_id)
    for (int n = 0; n < count_dbs; n++)     // Loop over all database sequences
      ...
      if (LQ > 2 * par.block_shading_space
          || length[backtrace_hits[n]] > 2 * par.block_shading_space) {
        ali_pos *res = new ali_pos[10];

        // Perform search step
        int num_res = swStripedWord_backtrace(LQ, first[backtrace_hits[n]],
            length[backtrace_hits[n]], gap_init, gap_extend, qw_it,
            workspace[thread_id], workspace[thread_id] + Ww,
            workspace[thread_id] + 2 * Ww, res);
...
#pragma omp critical
    ...
    if (print_elapsed)
      ElapsedTimeSinceLastCall("(SW backtrace prefilter)");
