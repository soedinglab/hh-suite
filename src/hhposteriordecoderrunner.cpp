    ...
    for (size_t idx = 0; idx < alignment.size(); idx++) {
        ...
        for(size_t idb = 0; idb < hits.size(); idb++){
            ...
            decoder->realign(*q_hmm, *t_hmm[current_thread_id],
                    ...
                    *m_backtrace_matrix[current_thread_id], alignment_to_exclude, par.exclstr, par.template_exclstr, par.min_overlap, par.shift, par.mact, par.corr, par.mac_min_length);
