...
void HitList::PrintMatrices(HMM* q, std::string outfile) {
  //limit matrices to par.max_number_matrices
  std::vector<bool> picked_alignments(matrices.size(), true);
  unsigned int chosen = matrices.size();

  if(matrices.size() == 0) {
    if(v > 0) {
      std::cerr << "There are no alignment matrices to print!" << std::endl;
    }
    return;
  }

  //remove duplicate alignments (mostly alignments which were realigned afterwards)
  for(int index1 = matrices.size() - 1; index1 >= 0; index1--) {
    if(picked_alignments[index1]) {
      for(int index2 = index1 - 1; index2 >= 0; index2--) {
        if(picked_alignments[index2] && matrices[index1]->id.compare(matrices[index2]->id) == 0) {
  ...
  float similarity_threshold_step = 0.01;

  //remove too similar alignments
  while (chosen > par.max_number_matrices) {
    for(unsigned int index = 0; index < matrices.size(); index++) {
      if(picked_alignments[index]) {
        for (unsigned int i = 0; i < matrices[index]->similarity_scores.size(); i++) {
          if (picked_alignments[i] && matrices[index]->similarity_scores[i] > similarity_threshold) {
            picked_alignments[index] = false;
            break;
          }
        }
      }
    ...
    similarity_threshold -= similarity_threshold_step;

    chosen = 0;
    for (std::vector<bool>::iterator it = picked_alignments.begin(); it != picked_alignments.end(); it++) {
      if((*it)) {
        chosen++;
      }
    }
  ...
  if(v > 1) {
    std::cout << "\n\n";
    std::cout << "Printing alignment matrices..." << std::endl;
    std::cout << "Total number of alignments    : " << matrices.size() << std::endl;
    std::cout << "Similarity threshold          : " << similarity_threshold << std::endl;
    std::cout << "Number of accepted alignments : " << chosen << std::endl;
    std::cout << "\n\n";
  }
  ...
  //TODO: unkown empty line in stdout
  std::ostream* out;
  if (par.matrixOutputFileName.compare("stdout") != 0) {
    out = new ofstream(par.matrixOutputFileName.c_str(),
  ...
  for (unsigned int index = 0; index < matrices.size(); index++) {
    ...
    Alignment_Matrices* it = matrices[index];

    const char* name = it->template_name.c_str();

    out->write(name, strlen(name));
    out->write(reinterpret_cast<const char*>(&delimiter_8_bit),
        sizeof(delimiter_8_bit));
    writeU16(*out, it->template_length);

    std::string id = it->id;

    unsigned char ali_probability = ali_probabilities.at(id);
    out->write(reinterpret_cast<const char*>(&ali_probability),
        sizeof(unsigned char));

    float lastForwardProbability = -1;
    ...
    for (unsigned short int i = 1; i <= q->L; i++) {
      ...
      if (it->forward_profile[i] > printForwardThreshold
        ...
        if (lastForwardProbability < printForwardThreshold) {
          writeU16(*out, i);
          lastForwardProbability = it->forward_profile[i];
        }
    ...
    float lastBackwardProbability = -1;
    ...
    for (unsigned short int i = 1; i <= q->L; i++) {
      ...
      if (it->backward_profile[i] > printBackwardThreshold
          && backwardProbability != 0) {
        if (lastBackwardProbability < printBackwardThreshold) {
          writeU16(*out, i);
          lastBackwardProbability = it->backward_profile[i];
        }
    ...
    int last_i = -1;
    int last_j = -1;
    for (std::vector<Posterior_Triple*>::iterator posterior_it =
        it->reduced_posterior_matrix->begin();
        posterior_it != it->reduced_posterior_matrix->end();
        posterior_it++) {

      float_to_8_bit((*posterior_it)->posterior_probability,
          posteriorProbability);

      if (last_i != (*posterior_it)->query_pos
          || last_j + 1 != (*posterior_it)->template_pos) {
        if (last_i != -1 && last_j != -1) {
          out->write(reinterpret_cast<const char*>(&delimiter_8_bit),
              sizeof(delimiter_8_bit));
        }
        writeU16(*out, (*posterior_it)->query_pos);
        writeU16(*out, (*posterior_it)->template_pos);
      }

      out->write(reinterpret_cast<const char*>(&posteriorProbability),
          sizeof(unsigned char));

      last_i = (*posterior_it)->query_pos;
      last_j = (*posterior_it)->template_pos;
    }
    out->write(reinterpret_cast<const char*>(&delimiter_8_bit),
        sizeof(delimiter_8_bit));
    writeU16(*out, delimiter_16_bit);
...
// 	     printf("hit=%-10.10s Lq=%-4i  Lt=%-4i  Nq=%5.2f  Nt=%5.2f  =>  lamda=%-6.3f  mu=%-6.3f\n",hit.name,q->L,hit.L,q->Neff_HMM,hit.Neff_HMM,lamda,mu);
    ...
    std::stringstream ss;
    ss << hit.name << "_" << hit.irep;
    std::string id = ss.str();

    ali_probabilities.insert(std::pair<std::string, float>(id, hit.Probab));
