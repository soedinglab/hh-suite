...
char VERSION_AND_DATE[]="version 2.1.0-pre (XXX 2013)";
char REFERENCE[]="Remmert M, Biegert A, Hauser A, and Soding J.\nHHblits: Lightning-fast iterative protein sequence searching by HMM-HMM alignment.\nNat. Methods 9:173-175 (2011).\n";
char COPYRIGHT[]="(C) Johannes Soeding, Michael Remmert, Andreas Biegert, Andreas Hauser\n";
...
void Parameters::SetDefaultPaths(char *program_path)
{
  ...
  if (testf) fclose(testf); 
    ...
    {
      if (v>=3) std::cerr<<"WARNING in HHsuite: Could not open "<<cs_library<<"\n";
  ...
  if (!testf)
    {
      std::cerr << std::endl << "Error in "<<argv[0]<<": could not find context_data.crf and cs219.lib in '" << hhlib << "'.\n"
...
void InitializePseudocountsEngine() {
  ...
  if (!fin) {
    std::cerr<<std::endl<<"Error in "<<par.argv[0]<<": could not open file \'"<<par.clusterfile<<"\'\n";
    exit(2);
  }
  char ext[100];
