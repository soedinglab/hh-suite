    ...
    for (j = jmin; j <= jmax; ++j) // Loop through template positions j
      ...
      if (cell_off[i][j]) {
        ...
        CALCULATE_MAX6( sMM_i_j, smin,
            ...
            sMI_i_1_j_1 + q->tr[i-1][M2M] + t->tr[j-1][I2M], bMM[i][j]);
...
void Hit::Forward(HMM* q, HMM* t, float** Pstruc) {
  ...
  for (j = 1; j <= t->L; ++j) {
    if (cell_off[1][j])
      F_MM_curr[j] = F_MI_curr[j] = F_DG_curr[j] = F_IM_curr[j] = F_GD_curr[j] =
          0.0;
    else {
      F_MM_curr[j] = ProbFwd(q->p[1], t->p[j]) * fpow2(ScoreSS(q, t, 1, j))
          * Cshift * (Pstruc == NULL ? 1 : Pstruc[1][j]);
      F_MI_curr[j] = F_DG_curr[j] = 0.0;
      F_IM_curr[j] = F_MM_curr[j - 1] * q->tr[1][M2I] * t->tr[j - 1][M2M]
          + F_IM_curr[j - 1] * q->tr[1][I2I] * t->tr[j - 1][M2M];
      F_GD_curr[j] = F_MM_curr[j - 1] * t->tr[j - 1][M2D]
          + F_GD_curr[j - 1] * t->tr[j - 1][D2D];
    }
  }
  ...
  for (i = 2; i <= q->L; ++i) // Loop through query positions i
    ...
    if (cell_off[i][jmin])
      ...
      F_MM_curr[jmin] = scale_prod * ProbFwd(q->p[i], t->p[jmin])
          * fpow2(ScoreSS(q, t, i, jmin)) * Cshift
          * (Pstruc == NULL ? 1 : Pstruc[i][jmin]);
    ...
    for (j = jmin + 1; j <= t->L; ++j) // Loop through template positions j
      ...
      if (cell_off[i][j])
        ...
        F_MM_curr[j] = substitutionScore * fpow2(ScoreSS(q, t, i, j))
            * Cshift * (Pstruc == NULL ? 1 : Pstruc[i][j]) * scale[i]
        ...
        F_GD_curr[j] = (F_MM_curr[j - 1] * t->tr[j - 1][M2D]         // GD -> MM
        + F_GD_curr[j - 1] * t->tr[j - 1][D2D]                    // GD -> GD
            + (Pstruc == NULL ?
                0 : F_DG_curr[j - 1] * t->tr[j - 1][M2D] * q->tr[i][D2M]) // DG -> GD (only when structure scores given)
        );
        F_IM_curr[j] = (F_MM_curr[j - 1] * q->tr[i][M2I] * t->tr[j - 1][M2M] // MM -> IM
        + F_IM_curr[j - 1] * q->tr[i][I2I] * t->tr[j - 1][M2M]     // IM -> IM
            + (Pstruc == NULL ?
                0 : F_MI_curr[j - 1] * q->tr[i][M2I] * t->tr[j - 1][I2M]) // MI -> IM (only when structure scores given)
        );
  ...
  for (i = 1; i <= q->L; ++i) // Loop through query positions i
    ...
    for (j = jmin; j <= jmax; ++j) // Loop through template positions j
      ...
      if (cell_off[i][j]) {
        ...
        CALCULATE_MAX4( S_curr[j], B_MM[i][j] - par.mact, // STOP signifies the first MM state, NOT the state before the first MM state (as in Viterbi)
...
void Hit::Backtrace(HMM* q, HMM* t) {
  ...
  for (i = 0; i <= q->L; ++i)
    bMM[i][1] = bGD[i][1] = bIM[i][1] = STOP;
  for (j = 1; j <= t->L; ++j)
    bMM[1][j] = bDG[1][j] = bMI[1][j] = STOP;
    ...
    switch (state) {
      ...
      case GD: // current state is GD
        switch (bGD[i][j--]) {
          case STOP:
            state = STOP;
            break; // current state does not have predecessor
          case MM:
            state = MM;
            break; // previous state is Match state
      ...
      default:
        fprintf(stderr,
            "Error in %s: unallowed state value %i occurred during backtracing at step %i, (i,j)=(%i,%i)\n",
  ...
  if (v >= 4) {
    ...
    for (step = nsteps; step >= 1; step--) {
      ...
      printf("%4i %4i     %2i %7.2f    ", this->i[step], this->j[step],
  ...
  for (step = 1; step <= nsteps; step++) {
    switch (states[step]) {
      case MM:
        ...
        //(par.useCSScoring && csSeq) ?
        //log2(columnStateScoring.substitutionScores[i][csSeq[j]]) :
        //Score(q->p[i], t->p[j]);
        S_ss[step] = ScoreSS(q, t, i, j, ssm);
        score_ss += S_ss[step];
...
void Hit::InitializeForAlignment(HMM* q, HMM* t, bool vit) {
  ...
  if (self) {
    ...
    if (par.block_shading && !strcmp(par.block_shading_mode, "tube")
        && par.block_shading->Contains(t->name)) {
