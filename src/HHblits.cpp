...
void HHblits::help(char all) {
	...
	if (all) {
		...
		printf(
				...
				par.psipred);
		printf(
				" -psipred_data <dir>  directory with PSIPRED data (default=%s) \n",
				par.psipred_data);
		printf("\n");
	}
	printf(
...
void HHblits::ProcessArguments(int argc, char** argv) {
	...
	for (int i = 1; i < argc; i++) {
		...
		else if (!strcmp(argv[i], "-pc_prefilter_contxt_mode")
				&& (i < argc - 1))
		...
		else if (!strcmp(argv[i], "-pc_prefilter_nocontxt_mode")
				&& (i < argc - 1))
...
int HHblits::PickBin(char status) {
...
void HHblits::PerformViterbiByWorker(int bin) {
	...
	if (!qa3mf) {
		...
		if (Qali.L != q->L) {
			cerr << "Error in " << par.argv[0] << ": " << par.infile << " has "
					<< q->L
					<< " match states, while its representative sequences have "
					<< Qali.L << "!\n";
			exit(1);
		}
...
void HHblits::DoViterbiSearch(char *dbfiles[], int ndb, bool alignByWorker) {
	// Search databases
...
//	    }
		...
		getTemplateHMM(dbfiles[idb], 1, hit[bin]->ftellpos, format[bin],
				t[bin]);
...
//		}
		...
		// if no multi-threading mode, main thread executes job itself
		if (alignByWorker)
			AlignByWorker(bin);
		else
...
void HHblits::ViterbiSearch(char *dbfiles[], int ndb, int db_size) {
...
void HHblits::perform_realign(char *dbfiles[], int ndb) {
	...
	while (!hitlist.End()) {
		...
		if (nhits >= par.premerge) // realign the first premerge hits consecutively to query profile
			...
			if (hit_cur.irep == 1) {
				...
				phash_plist_realignhitpos->Show(hit_cur.dbfile)->Push(
						realign_hitpos);
			}
			...
			array_plist_phits[hit_cur.index]->Push(
					hitlist.ReadCurrentAddress());
		}
	...
	if (Lmax > Lmaxmem) {
		Lmax = Lmaxmem;
		if (v >= 1) {
			cerr << "WARNING: Realigning sequences only up to length "
					<< Lmaxmem << "." << endl;
			cerr
	...
	if (par.premerge > 0) {
		...
		while (!hitlist.End() && nhits < par.premerge) {
			...
			getTemplateHMM(hit_cur.dbfile, par.wg, hit[bin]->ftellpos,
					format[bin], t[bin]);
			...
			if (par.allseqs) // need to keep *all* sequences in Qali_allseqs? => merge before filtering
				Qali_allseqs.MergeMasterSlave(*hit[bin], Tali,
						hit[bin]->dbfile);
	...
	for (int idb = 0; idb < ndb; idb++) {
		...
		while (!phash_plist_realignhitpos->Show(dbfiles[idb])->End()) {
			...
			while (!phash_plist_realignhitpos->Show(dbfiles[idb])->End()
				...
				hit[bin]->dbfile = new char[strlen(dbfiles[idb]) + 1];
				strcpy(hit[bin]->dbfile, dbfiles[idb]); // record db file name from which next HMM is read
				getTemplateHMM(dbfiles[idb], par.wg, hit[bin]->ftellpos,
						format[bin], t[bin]);
...
void HHblits::recalculateAlignmentsForDifferentQSC(HitList& hitlist,
	...
	for (size_t qsc_index = 0; qsc_index < nqsc; qsc_index++) {
		...
		qali.N_filtered = qali.Filter(par.max_seqid, cov_tot, par.qid,
		...
		realigned_viterbi_hitlist.CalculatePvalues(q);
		realigned_viterbi_hitlist.CalculateHHblitsEvalues(q);
...
void HHblits::wiggleQSC(HitList& hitlist, int n_redundancy, Alignment& Qali,
	...
	recalculateAlignmentsForDifferentQSC(*reducedHitList, Qali, inputformat,
			qsc, nqsc, reducedFinalHitList);  //*wiggledHitList);
...
void HHblits::checkCSFormat(size_t nr_checks) {
	...
	if (nr_checks == 0) {
		std::cerr << "Error: Your cs database is in an old format!"
				<< std::endl;
				...
				if (j >= LQ)
					...
					float dummy = flog2(query_profile[j + 1][a])
							* par.prefilter_bit_factor
							+ par.prefilter_score_offset + 0.5;
...
//  int* prefiltered_hits = new int[par.dbsize+1];
	...
	if (print_elapsed)
		ElapsedTimeSinceLastCall("(init prefiltering)");
...
#pragma omp critical
	...
	if (print_elapsed)
		ElapsedTimeSinceLastCall("(ungapped preprefilter)");
...
#pragma omp critical
	...
	vector<pair<double, int> >::iterator second_prefilter_begin_erase =
			hits.end();
	vector<pair<double, int> >::iterator second_prefilter_end_erase =
			hits.end();
	count_dbs = 0;
	...
	if (print_elapsed)
		ElapsedTimeSinceLastCall("(SW prefilter)");
...
//				if (par.prefilter)
//					alpha = par.alphaa
//							+ par.alphab * (hit_neff - 1)
//									* (1 - par.alphac * (q_neff - 1));
//
//				hit[bin]->Eval = exp(
//						hit[bin]->logPval + log_dbsize + (alpha * log_Pcut));
//				hit[bin]->logEval = hit[bin]->logPval + log_dbsize
//						+ (alpha * log_Pcut);
//
//				// Rolling average: replace oldest data point at par.filter_counter by newest one
//				early_stopping->sum -=
//						early_stopping->evals[early_stopping->counter];
//				early_stopping->evals[early_stopping->counter] = 1.0
//						/ (1.0 + hit[bin]->Eval);
//				early_stopping->sum +=
//						early_stopping->evals[early_stopping->counter];
//
...
//				if (early_stopping->counter == early_stopping->length)
//					early_stopping->counter = 0;
//			}
//		}

		if (par.forward > 0)
			break; // find only best alignment for forward algorithm and stochastic sampling
		if (hit[bin]->score <= SMIN)
			break; // break if score for previous hit is already worse than SMIN
	}
	return;
}
...
int HHblits::run(int argc, char **argv) {
	int cluster_found = 0;
	int seqs_found = 0;
	...
	if (use_compressed_a3m) {
		...
		dbuniprot_sequence_data_file = fopen(dbuniprot_sequence_data_filename,
				"r");
		...
		dbuniprot_sequence_data = ffindex_mmap_data(
				dbuniprot_sequence_data_file, &uniprot_sequence_data_offset);

		//uniprot sequences database
		dbuniprot_header_data_file = fopen(dbuniprot_header_data_filename, "r");
		dbuniprot_header_index_file = fopen(dbuniprot_header_index_filename,
				"r");
		...
		dbuniprot_header_index = ffindex_index_parse(
				dbuniprot_header_index_file, uniprot_header_data_size);
		...
		if (!*dba3m_data_filename) {
			...
			if (dba3m_index == NULL) {
				cerr << "Error in " << par.argv[0]
						<< ": could not read index file" << dba3m_index_filename
						<< ". Is the file empty or corrupted?\n";
				exit(1);
			}
	...
	for (int round = 1; round <= num_rounds; round++) {
		...
		*q_tmp = *q;
		...
		if (round < num_rounds || *par.alnfile || *par.psifile || *par.hhmfile
			...
			if (new_hits != 0) {
				...
				while (!hitlist.End()) {
					...
					if (par.allseqs) // need to keep *all* sequences in Qali_allseqs? => merge before filtering
						Qali_allseqs.MergeMasterSlave(hit_cur, Tali,
								hit_cur.dbfile);
					Tali.N_filtered = Tali.Filter(par.max_seqid_db,
							par.coverage_db, par.qid_db, par.qsc_db,
							par.Ndiff_db);
				...
				Qali.FilterForDisplay(par.max_seqid, par.coverage, par.qid,
						par.qsc, par.nseqdis);

				// Remove sequences with seq. identity larger than seqid percent (remove the shorter of two)
				float const COV_ABS = 25;     // min. number of aligned residues
				int cov_tot = imax(
						imin((int) (COV_ABS / Qali.L * 100 + 0.5), 70),
						par.coverage);
				if (v > 2)
			...
			if (print_elapsed)
				ElapsedTimeSinceLastCall(
						"(Calculate AA frequencies and transitions)");
		...
		if (v >= 2
				&& (round < num_rounds || *par.alnfile || *par.psifile
						|| *par.hhmfile || *alis_basename))
			printf(
	...
	if (*par.pairwisealisfile) {
		if (v >= 2)
			cout << "Printing alignments in "
					<< (par.outformat == 1 ? "FASTA" :
						par.outformat == 2 ? "A2M" : "A3M") << " format to "
					<< par.pairwisealisfile << "\n";
	...
	if (strlen(par.reduced_outfile) != 0) {
		...
		wiggleQSC(hitlist, 10, Qali, input_format, q->L, qscs, 4,
				reducedHitlist);
