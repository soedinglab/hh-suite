#! /usr/bin/perl -w
# Merge multiple alignments in A3M format via a multiple alignment of their seed/query sequences.
...
my $v=2;     # 3: DEBUG
my $help="
 Merge multiple alignments in A3M format via a multiple alignment of (parts of) their seed sequences.
 The output file is generated in A3M format.  The multiple alignments in A3M format must have a match state 
 assignment by their seed (=first) sequence (as produced by buildali.pl or reformat.pl with option -M first).
 DSSP state sequences will be included by default, and a consensus DSSP state sequence appended at the top.
 Usage: $program infile.fas outfile [options]
 ...
 -d <dirs>    directories (separated with spaces) where to find the *.a3m alignments (default='.')
 -mark        mark the seed sequences of the superalignment in their nameline by inserting a \@: >\@name 
 -diff <int>  use only the <int> most different sequences from each alignment for merging (calls hhfilter) 
 -name <name> write a name line '#name' as first line into a3m file
 -full        for the first seed sequence, use the full-length sequence from the alignment (including leading and trailing residues)
...
if ($full) {
	...
	if (-e $alifile) {last;}
	if (-e $alifilefas) {
	    &System("perl $perl/reformat.pl -M first $alifilefas $alifile");
	    last;
	}   
	$alifilefas="";
    }
    ...
    if ($diff ne "") {
	&System("$hh/hhfilter -v $v -i $alifile -o $tmpfile -diff $diff -cov 5");
	$alifile=$tmpfile;
    } 
...
#&System("perl $perl/reformat.pl $baseout.a3m $baseout.fas -v 3");
# Reformat $baseout.a3m such that match columns are defined by first seed sequence?
#&System("perl $perl/reformat.pl -M first $baseout.fas $baseout"."_first.a3m");
