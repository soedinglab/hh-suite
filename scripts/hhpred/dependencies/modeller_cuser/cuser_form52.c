...
// compile:  gcc -shared -Wall -fPIC `/cluster/user/armin/bin/modeller9.10/bin/mod9.10 --cflags --libs` `pkg-config --cflags glib-2.0` -I/usr/include/python2.6 cuser_form52.c cuser_form52_wrap.c -o _cuser_form52.so -lm


#include <glib.h>
#include "modeller.h"
#include "cuser_form52.h"
#include <stdio.h>
#include <math.h>
#include <stdlib.h>
                       ...
                       int n_pcsr, gboolean deriv, float *fderv, float *val)
{
  ...
  *val = RT * (-log(com + 1 - w) );		
  ...
  if (deriv) {
    fderv[0] = RT * ((-1) * (com * argBar) / (com + 1 - w));
  }
