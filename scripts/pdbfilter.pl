...
my $Evalmin=1e8;   # BLAST E-value
my $covmin=90;     # minimum coverage threshold
my $v=2;

my $help="
 Read pdb or SCOP sequences from infile and write representative set of sequences to outfile
 Compare each sequence with all others. If more than \$covmin\% of the sequence with lower 
 resolution is covered by the alignment AND the sequence identity is larger than \$idmin 
 AND the E-value is better than \$Evalmin then throw out the sequence with lower resolution. 
